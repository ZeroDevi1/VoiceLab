...
def _normalize_wav_path(path: str) -> str:
    ...
    if m:
        ...
        mnt_root = os.environ.get("WSL_MNT_ROOT", "/mnt").rstrip("/")
        candidate = re.sub(r"/+", "/", f"{mnt_root}/{drive}/{rest}")
...
if __name__ == "__main__":
    ...
    with open('{}/wav.scp'.format(args.dir)) as f:
        for l in f:
            l = l.replace('\n', '').split()
            utt2wav[l[0]] = _normalize_wav_path(l[1])

    option = onnxruntime.SessionOptions()
    option.graph_optimization_level = onnxruntime.GraphOptimizationLevel.ORT_ENABLE_ALL
    option.intra_op_num_threads = 1
    force_cpu = os.environ.get("COSYVOICE_ORT_FORCE_CPU", "").lower() in {"1", "true", "yes", "y"}
    providers = ["CPUExecutionProvider"] if force_cpu else ["CUDAExecutionProvider", "CPUExecutionProvider"]
    try:
        ort_session = onnxruntime.InferenceSession(args.onnx_path, sess_options=option, providers=providers)
    except Exception as exc:
        logging.warning("Failed to create CUDA onnxruntime session, falling back to CPU. exc=%s", exc)
